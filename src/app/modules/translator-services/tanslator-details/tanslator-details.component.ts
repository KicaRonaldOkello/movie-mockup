...
export class TanslatorDetailsComponent implements OnInit {
  ...
  upload() {
    const myWidget = cloudinary.createUploadWidget({
        ...
        if (!error && result && result.event === 'success') {
          this.liveTranslationForm.patchValue({attachment: result.info.secure_url});
        } else if (error) {
  ...
  payNow() {
    ...
    this.isLiveTranslation ? data['attachments'] = this.liveTranslationForm.get('attachment').value : null;
