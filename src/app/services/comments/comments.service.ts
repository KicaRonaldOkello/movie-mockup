...
export class CommentsService {
  ...
  getComments(commentId):Observable<any> {
    return this.http.get(
      `https://hebertazurefunctions.azurewebsites.net/api/GetAllPageComments?PageId=${commentId}&code=tjNVpZgpeDGDhSfGWpOHsV25A0T7EFOzkPigTNsoE7NpLQxfqtEIRA==`,{headers:{skip:"true"} }
    ).pipe(
      map(data => data)
    )
  }
